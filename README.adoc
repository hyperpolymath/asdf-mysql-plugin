...
:url-repo: https://github.com/hyperpolymath/asdf-mysql-plugin
...
image:https://img.shields.io/github/license/hyperpolymath/asdf-mysql-plugin?style=flat-square[License,link=LICENSE]
image:https://img.shields.io/github/v/release/hyperpolymath/asdf-mysql-plugin?style=flat-square[Release,link={url-repo}/releases]
image:https://img.shields.io/github/actions/workflow/status/hyperpolymath/asdf-mysql-plugin/ci.yml?branch=main&style=flat-square[Build Status,link={url-repo}/actions]
image:https://img.shields.io/badge/asdf-plugin-blue?style=flat-square[asdf Plugin,link={url-asdf}]

[.lead]
An {url-asdf}[asdf] plugin to install and manage {url-tool}[Mysql] versions.
...
Mysql is an upstream tool managed by this plugin. Refer to the official
...
|**Version Switching**
|Switch between Mysql versions per project

|**Project Isolation**
|Pin Mysql per repository via `.tool-versions`

|**Reproducible Builds**
|Align CI/CD and developer environments to the same Mysql version
...
=== Installing Mysql Versions

[source,bash]
----
asdf list all mysql
asdf install mysql latest
asdf install mysql 0.1.0
----
...
[source,bash]
----
asdf global mysql 0.1.0
asdf local mysql 0.1.0
asdf current mysql
mysql --version
----
...
* Ensure `mysql` is on PATH after installation.
* Verify the installed version with `mysql --version`.
